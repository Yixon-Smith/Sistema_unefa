<?php
	class Modelo_ciuadano
	{
		...
		function Registrar_ciudadano($nombre,$apePat,$apeMat,$matricula,$telefo,$movil,$direcc,$fecnac,$dni,$email,$genero,$tsangre){
			$sql = "call PA_REGISTRARCIUDADANO('$nombre','$apePat','$apeMat','$matricula','$telefo','$movil','$direcc','$fecnac','$dni','$email','$genero','$tsangre')";
		...
		function Editar_ciudadano($codigo,$nombre,$apePat,$apeMat,$matricula,$telefo,$movil,$direc,$fecha,$nrodocume,$email){
			$sql = "call PA_EDITARCIUDADANOTODOS('$codigo','$nombre','$apePat','$apeMat','$matricula','$telefo','$movil','$direc','$fecha','$nrodocume','$email')";
			if ($resultado = $this->conexion->conexion->query($sql)){
				return 1;
			}
			else{
